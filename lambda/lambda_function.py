...
def lambda_handler(event, context):
    ...
    # Generate distinct temp file paths safely
    ...
    try:
        ...
        # Ensure input was in uploads/ prefix to avoid replacing wrong strings
        output_key = key.replace("uploads/", "processed/")

        s3.upload_file(output_pdf, bucket, output_key)

        return {"statusCode": 200, "body": "Watermark Applied"}

    finally:
        # Explicitly clean up /tmp files to prevent disk space exhaustion
        for path in [input_pdf, watermark_pdf, output_pdf]:
